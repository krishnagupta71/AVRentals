...
export function AdminTrips () {
  ...
  const locationMap = useMemo(() => keyBy(locations, ({waypoint}) => waypoint), [locations]);
  const tableTrips = useMemo(() => trips.map((trip) => tripToTableTrip(trip, locationMap)), [trips, locationMap])
  const locationData = useMemo(() => {
    const allLocations = Array.from(
      new Set(tableTrips.flatMap((trip) => [trip.pickup, trip.dropoff])));
    const results = Object.fromEntries(allLocations.map((name) => ([name,{name, pickup: 0, dropoff: 0}])));
